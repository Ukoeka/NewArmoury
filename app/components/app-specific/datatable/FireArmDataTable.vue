...
export const schema = z.object({
  ...
  make: z.string(),
  model: z.string(),
  category: z.enum(["Rifle", "Pistol", "Shotgun", "SMG", "Other"]),
  caliber: z.string(),
  manufacturer: z.string().optional(),
  acquisitionDate: z.string(), // ISO date
  status: z.enum(["In Storage", "Assigned", "Under Maintenance", "Disposed", "Lost"]),
...
export interface Firearm {
  ...
  make: string
  model: string
  category: "Rifle" | "Pistol" | "Shotgun" | "SMG" | "Other"
  caliber: string
  manufacturer: string
  acquisitionDate: string // ISO date
  status: "In Storage" | "Assigned" | "Under Maintenance" | "Disposed" | "Lost"
...
const columns: ColumnDef<Firearm>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => h("div", String(row.original.serialNumber)),
    enableHiding: false,
  },
   {
    accessorKey: "make",
    header: "Make",
    cell: ({ row }) => h("div", String(row.original.make)),
    enableHiding: true,
  },
  {
    accessorKey: "model",
    header: "Model",
    cell: ({ row }) => h("div", String(row.original.model)),
    enableHiding: true,
  },
   {
    accessorKey: "category",
    header: "Category",
    cell: ({ row }) => h("div", String(row.original.category)),
    enableHiding: false,
  },
  {
    accessorKey: "caliber",
    header: "Caliber",
    cell: ({ row }) => h("div", String(row.original.caliber)),
    enableHiding: false,
  },
  {
    accessorKey: "manufacturer",
    header: "Manufacturer",
    cell: ({ row }) => h("div", String(row.original.manufacturer)),
    enableHiding: true,
  },
  {
    accessorKey: "condition",
    header: "Condition",
    cell: ({ row }) => h("div", String(row.original.condition)),
    enableHiding: true,
  },
 {
    accessorKey: "status",
    ...
    cell: ({ row }) => {
      ...
      const statusColorMap: Record<string, string> = {
        "In Storage": "bg-slate-800",
        "Assigned": "bg-emerald-800",
        "Under Maintenance": "bg-amber-800",
        "Disposed": "bg-red-800",
        "Lost": "bg-red-900",
      }
