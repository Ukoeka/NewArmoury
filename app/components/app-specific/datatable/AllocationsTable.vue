...
export interface Allocation {
  id: number
  referenceId: string,
  itemType: "Firearm" | "Ammunition" | "Kit"
  itemName: string
  quantity: number
  allocatedTo: string
...
const columns: ColumnDef<Allocation>[] = [
  { id: "drag", header: () => null, cell: ({ row }) => h(DragHandle) },

  {
    id: "select",
    header: ({ table }) => h(Checkbox, {
      modelValue: table.getIsAllPageRowsSelected() || (table.getIsSomePageRowsSelected() && "indeterminate"),
      "onUpdate:modelValue": val => table.toggleAllPageRowsSelected(!!val),
    }),
    cell: ({ row }) => h(Checkbox, {
  ...
  { accessorKey: "quantity", header: "Qty", cell: ({ row }) => String(row.original.quantity) },

  { accessorKey: "allocatedTo", header: "Issued To" },

  { accessorKey: "requestReason", header: "Reason" },

  {
    accessorKey: "allocatedDate",
    header: "Issued Date",
    cell: ({ row }) => new Date(row.original.allocatedDate).toLocaleDateString()
  },

  {
    ...
    cell: ({ row }) =>
      row.original.expectedReturn
        ? new Date(row.original.expectedReturn).toLocaleDateString()
        : "N/A"
  },
      ...
      h(DropdownMenu, {}, {
        default: () => [
          ...
          h(DropdownMenuContent, { align: "end" }, () => [
            ...
            h(DropdownMenuItem, {}, () => "Edit Allocation"),
            h(DropdownMenuSeparator),
            h(DropdownMenuItem, { class: "text-red-600" }, () => "Revoke / Delete"),
          ])
        ]
      }),
  }
