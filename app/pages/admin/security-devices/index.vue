<template>
  <div class="p-6 min-h-screen bg-[#0A0E1A] text-slate-200 font-sans">
    ...
    <!-- Filter Bar -->
    <div class="bg-[#161b27] border border-[#1e2535] rounded-xl px-4 py-3 flex flex-col sm:flex-row gap-3 items-center mb-5">
      <div class="relative flex-1">
        <Search :size="14" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600 pointer-events-none" />
        <input v-model="searchQuery" type="text" placeholder="Search by name, ID, or type..."
    ...
    <AddEdit
      ...
      mode="edit"
      :initial-data="selectedDevice ? deviceToForm(selectedDevice) : undefined"
      @submit="handleEdit"
    />
    <DeviceInspection
      v-model:open="showInspectionModal"
      :device="selectedDevice ? { deviceName: selectedDevice.name, deviceId: selectedDevice.id } : null"
      @submit="handleInspection"
    />
    <DeviceDetails
      v-model:open="showDetailsModal"
      :device="selectedDevice ? deviceToDetail(selectedDevice) : null"
      @edit="editingId = selectedDevice?.id ?? null; showDetailsModal = false; showEditModal = true"
      @inspect="inspectingId = selectedDevice?.id ?? null; showDetailsModal = false; showInspectionModal = true"
    />
...
interface Device {
  ...
  branch: string; status: 'GOOD' | 'MALFUNCTION' | 'MAINTENANCE'
  nextInspection: string; daysUntil: number
  number?: string; location?: string
  lastInspection?: string; inspectedBy?: string; inspectionRemarks?: string
}
...
const devices = ref<Device[]>([
  { id: 'WTMD-DSM-001', name: 'Walk-Through Metal Detector',    type: 'Metal Detector',        use: 'Entrance Security Screening',      branch: 'Dar es Salaam Sub-HQ', status: 'GOOD',        nextInspection: '1/20/2025',  daysUntil: -410, lastInspection: '12/20/2024', inspectedBy: 'u-004', inspectionRemarks: 'All zones functioning properly. Sensitivity calibrated.' },
...
// Locked-in IDs so modal transitions (details → edit) don't lose the reference
...
function deviceToForm(d: Device) {
  return {
    deviceName: d.name, deviceId: d.id, type: d.type,
    number: d.number ?? '', purpose: d.use,
    location: d.location ?? '', branch: d.branch,
    status: d.status === 'GOOD' ? 'Good' : d.status === 'MAINTENANCE' ? 'Maintenance Required' : 'Faulty',
  }
}

function deviceToDetail(d: Device) {
  return {
    id: 0, deviceName: d.name, deviceId: d.id, type: d.type,
    number: d.number ?? '—', purpose: d.use, location: d.location ?? '—', branch: d.branch,
    status: d.status === 'GOOD' ? 'Good' : d.status === 'MAINTENANCE' ? 'Maintenance Required' : 'Faulty',
    lastInspection: d.lastInspection, nextInspection: d.nextInspection,
    inspectedBy: d.inspectedBy, inspectionRemarks: d.inspectionRemarks,
  }
}
...
function openInspection(d: Device) {
  selectedDevice.value = d
  inspectingId.value = d.id
  showInspectionModal.value = true
}
function openDetails(d: Device) {
  selectedDevice.value = d
...
function handleAdd(data: any) {
  devices.value.push({
    id: data.deviceId, name: data.deviceName, type: data.type,
    use: data.purpose, branch: data.branch,
...
function handleEdit(data: any) {
  ...
  devices.value[idx] = {
    ...devices.value[idx],
    name: data.deviceName, type: data.type, use: data.purpose,
    branch: data.branch, number: data.number, location: data.location,
    status: data.status === 'Good' ? 'GOOD' : data.status === 'Maintenance Required' ? 'MAINTENANCE' : 'MALFUNCTION',
  }
...
function handleInspection(data: any) {
  const idx = devices.value.findIndex(d => d.id === inspectingId.value)
  if (idx === -1) return
  devices.value[idx] = {
    ...devices.value[idx],
    status: data.status === 'Good' ? 'GOOD' : data.status === 'Maintenance Required' ? 'MAINTENANCE' : 'MALFUNCTION',
    lastInspection: new Date().toLocaleDateString(),
    inspectionRemarks: data.remarks,
