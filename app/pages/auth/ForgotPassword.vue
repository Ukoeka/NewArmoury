  ...
  <div class="min-h-screen bg-[#0A0E1A] flex">
    ...
    <div class="flex flex-col w-full lg:w-120 xl:w-130 shrink-0 px-8 py-10 md:px-14">
      ...
      <NuxtLink to="/" class="flex items-center gap-2.5 group w-fit">
        ...
        <span class="text-[15px] font-bold tracking-tight text-slate-100 group-hover:text-white transition-colors">
          {{ appName }}
        </span>
      </NuxtLink>
      ...
      <div class="flex flex-1 items-center justify-center">
        <div class="w-full max-w-sm">
          ...
          <!-- Back link -->
          <NuxtLink to="/auth/login" class="inline-flex items-center gap-1.5 text-[13px] text-slate-500 hover:text-slate-300 transition-colors mb-8 group">
          ...
          <div class="mb-8">
            ...
            <h1 class="text-[26px] font-bold text-slate-100 tracking-tight mb-1.5">
              Reset your password
            </h1>
          ...
          <form class="flex flex-col gap-5" @submit.prevent="handleSubmit">
            <div class="flex flex-col gap-1.5">
              <label for="reset-email" class="text-[13px] font-medium text-slate-300">
                Email address
              </label>
              <input
                id="reset-email"
                ...
                required
                class="h-10 w-full rounded-lg border border-[#1e2535] bg-[#161b27] px-3.5 text-[14px] text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-blue-500/60 focus:ring-1 focus:ring-blue-500/30 transition-colors"
              />
            </div>
          ...
          <!-- Footer link -->
          <p class="mt-8 text-center text-[13px] text-slate-600">
    ...
    <div class="hidden lg:flex flex-1 relative overflow-hidden bg-[#070b14] border-l border-[#1e2535]">
      ...
      <div class="relative z-10 flex flex-col justify-center w-full p-14 gap-10">
        ...
        <div>
          ...
          <h2 class="text-[28px] font-black text-slate-100 tracking-tight leading-snug max-w-xs">
            Secure Access<br>
            <span class="text-blue-500">Recovery</span> Process
          </h2>
          <p class="mt-3 text-[14px] text-slate-500 leading-relaxed max-w-xs">
        ...
        <div class="flex flex-col gap-5">
          <div v-for="(step, i) in steps" :key="i" class="flex items-start gap-4">
            <div class="w-7 h-7 rounded-lg bg-[#161b27] border border-[#1e2535] flex items-center justify-center shrink-0 mt-0.5">
              <span class="text-[11px] font-bold text-slate-400">{{ i + 1 }}</span>
            </div>
            <div>
              <div class="text-[13px] font-semibold text-slate-200">{{ step.title }}</div>
              <div class="text-[12px] text-slate-500 mt-0.5">{{ step.desc }}</div>
            </div>
          </div>
        </div>
