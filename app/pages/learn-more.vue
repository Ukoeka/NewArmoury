  ...
  <div class="max-w-6xl mx-auto px-6 py-16 flex flex-col gap-16">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
      <div
        v-for="f in features"
        :key="f.title"
        class="bg-[#161b27] border border-[#1e2535] rounded-xl p-6 flex flex-col gap-4"
      >
        <div class="flex items-center gap-3">
          ...
          <span class="text-[15px] font-semibold text-slate-100">{{ f.title }}</span>
        </div>
        <p class="text-[13.5px] text-slate-400 leading-relaxed">{{ f.description }}</p>
    ...
    <!-- Roles Section -->
    <div class="bg-[#161b27] border border-[#1e2535] rounded-xl p-8 flex flex-col gap-6">
      <div>
        <h2 class="text-[20px] font-bold text-slate-100 mb-2">Role-based access control</h2>
        <p class="text-[14px] text-slate-400">Eight distinct roles ensure every user sees and does only what they're authorized for.</p>
      </div>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        ]" :key="role.code" class="bg-[#1D293D] border border-[#1e2535] rounded-lg px-4 py-3">
          <div class="text-[11px] font-bold text-blue-400 tracking-wider mb-1">{{ role.code }}</div>
          <div class="text-[12px] text-slate-400">{{ role.label }}</div>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col items-center gap-5 py-6 text-center">
      <h2 class="text-[22px] font-bold text-slate-100">Ready to get started?</h2>
      <p class="text-[14px] text-slate-400 max-w-md">Access the portal using your assigned credentials. Contact your system administrator if you need access.</p>
